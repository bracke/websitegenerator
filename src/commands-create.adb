...
package body Commands.Create is
   ...
   begin
      if Args.Length > 0 then
         declare
            Path : constant String := Args.First_Element;
            ToDo : Action := Write;
         begin
            if Cmd.Dry_Run then
            ...
            if Dir.Kind (Path) = Ada.Directories.Directory
            then
               Init_Project.Init (Path,
               Cmd.Blueprint.all, ToDo);
            else
               IO.Put_Line (TT.Error (Path &
               " exists and is not a directory."));
            end if;
         end;
      else
         IO.Put_Line (TT.Error (
            "Command requires a project name to be specified."));
      end if;
